...
def probe(imol):
  ...
  if is_valid_model_molecule(imol):
    ...
    if not reduce_command:
      ...
      print("BL INFO:: running reduce: %s %s and ouptut to: %s" %(reduce_command , arg_list, reduce_out_pdb_file))
...
def get_probe_dots_from(pdb_file_name, point, radius):
  ...
  if (interactive_probe_is_OK_qm == 'yes'):
    ...
    within_str = "(within " + str(radius) + " of " + str(point[0]) + ", " + str(point[1]) + ", " + str(point[2]) + ")"
