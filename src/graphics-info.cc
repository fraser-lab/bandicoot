...
#endif // HAVE_GTK_CANVAS (outer guard for function definition)
...
#ifdef __APPLE__
      // Viewport is in physical pixels on Retina, so a line width of 1.0
      // becomes a sub-logical-pixel hairline -- stroke labels render but
      // are effectively invisible. Bump line width to the backing factor.
      glLineWidth(2.0);
#else
      glLineWidth(1.0);
#endif
...
#ifdef __APPLE__
      // glutStrokeCharacter is silently broken in the macOS freeglut build,
      // so on Apple we render the string into a native NSString-drawn
      // texture and project it as a textured quad in the same transformed
      // space. The current matrix already orients the quad to face the
      // camera and positions it at the label anchor.
      {
	 // Map Coot's atom_label_font_size preference (1=small,
	 // 2=medium, 3=large; 4-9 are GLUT bitmap fallbacks we treat
	 // similarly) to a point size for native rendering.
	 double pt;
	 switch (graphics_info_t::atom_label_font_size) {
	 case 1: pt = 104.0; break;
	 case 3: pt = 192.0; break;
	 case 4: pt = 96.0;  break;
	 case 5: pt = 200.0; break;
	 case 6: pt = 112.0; break;
	 case 7: pt = 128.0; break;
	 case 8: pt = 80.0;  break;
	 case 9: pt = 224.0; break;
	 case 2:
	 default: pt = 144.0; break;
	 }
	 int tw = 0, th = 0;
	 unsigned int tex = bandicoot_make_text_texture(s.c_str(), pt, &tw, &th);
	 if (tex && tw > 0 && th > 0) {
	    ...
	    // Tint the white-on-transparent texture with the configured
	    // label colour.
	    glColor4f(graphics_info_t::font_colour.red,
		      graphics_info_t::font_colour.green,
		      graphics_info_t::font_colour.blue, 1.0f);
	    // NSBitmapImageRep rows are top-down; flip V so the texture
	    // isn't upside down in the quad.
	    glBegin(GL_QUADS);
	    glTexCoord2f(0.0f, 1.0f); glVertex3f(0.0f,        0.0f,        0.0f);
	    glTexCoord2f(1.0f, 1.0f); glVertex3f((float)tw,   0.0f,        0.0f);
	    glTexCoord2f(1.0f, 0.0f); glVertex3f((float)tw,   (float)th,   0.0f);
	    glTexCoord2f(0.0f, 0.0f); glVertex3f(0.0f,        (float)th,   0.0f);
	    glEnd();
	    glPopAttrib();
