...
#ifdef __APPLE__
// On macOS the bitmap-text path (glRasterPos3f + glutBitmapCharacter) renders
// invisibly through GtkGLExt-Quartz's backing layer -- atom labels and axis
// x/y/z markers never appear. The stroke-characters path uses regular 3D GL
// primitives (glutStrokeCharacter inside a glPushMatrix transform) and works.
bool      graphics_info_t::stroke_characters = true;
#else
...
int graphics_info_t::rama_psi_axis_mode = 0; // coot::rama_plot::PSI_CLASSIC
...
// 				     gl_context_y_size);
	...
	// Default the main window to 1/3 of the screen width by 1/2 the
	// screen height — works across laptops and large displays without
	// hardcoded pixels. Falls back to the historic GL-context-sized
	// window if the screen can't be queried.
...
#ifdef __APPLE__
   // On macOS, explicitly promote the GL drawing area's GdkWindow to a
   // native NSView. GDK_NATIVE_WINDOWS=1 alone is unreliable on Tahoe;
   // without this call, app-wide event routing (menu / toolbar / dialog
   // clicks) silently fails even though widgets render correctly.
   // coot-deps relies on the same call (via quartz_fix.m::ensure_native_gl_window).
   if (widget->window) {
...
gint glarea_button_press(GtkWidget *widget, GdkEventButton *event) {
...
#ifdef __APPLE__
   // GTK-Quartz on Tahoe fires multiple button_press events (and synthetic
   // 2BUTTON_PRESS / 3BUTTON_PRESS) for a single physical click. That makes
   // any toggle-style handler (atom labeling, which add-or-removes from the
   // labelled list) misbehave -- it ends up adding-then-removing on each
   // physical click. Dedupe by time: drop events arriving within 100 ms of
   // the previous one for the same button.
   static gint64 last_press_us = 0;
   static guint  last_button   = 0;
   gint64 now_us = g_get_monotonic_time();
   if (last_press_us > 0 && event->button == last_button &&
   ...
   // GTK-Quartz on Tahoe doesn't populate event->state with modifier bits
   // (shift/control); only the button mask makes it through. Query AppKit
   // directly so shift-click (atom label) and ctrl-modifier paths work.
#ifdef __APPLE__
