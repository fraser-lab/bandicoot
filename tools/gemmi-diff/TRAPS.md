...
Lives **outside the repository**, in a sibling directory of the checkout -- deliberately
(2026-08-12): these are large binary-ish files that would bloat git history
...
3NYD, 5E1N, 6DMH). Three groups cannot: the phenix.refine output pair, the
`*_hierarchy.cif` files from the encoding partner, and `4517425.cif` from the COD.
(The `*_hierarchy.cif` set is citable: the encoding is public at
`github.com/diff-use/mmcif_encoding`.)

**A dozen trap entries below cite "the phenix.refine pair", and it is NO LONGER IN THE
CORPUS** -- it was removed when the corpus was reorganised, after checking that its
coverage was replaced (1AON covers the multi-block case, 1FFK the hydrogen-bond one with
2870 `hydrog` rows against the pair's 660). Those entries are therefore **documentary
rather than re-runnable**: their measured numbers stand as the record of what was found
and why the code is shaped as it is, but pointing the harness at the corpus today will not
reproduce them. Everything cited by accession code still will.
...
**Exhibited by:** the phenix.refine mmCIF pair -- block 1 is
the structure, block 2 is `data_comp_CL`, a monomer restraint
...
`UnitCell` is `1,1,1,90,90,90` -- so a cell-less model silently acquires a fake 1 A cubic
...
**WARNING: The fallback order is a correctness matter, not a preference:** Hall symbol (unambiguous),
...
**WARNING: gemmi's DEFAULT `which=7` IS WRONG HERE.** `make_structure_from_chemcomp_block(block, which)`
...
those categories away. Measured on one of the phenix pair: preserved backup carries
`_struct_ncs_oper` x15, `mmdb ReadCoorFile` fails.
**WARNING: THE WRITE-SIDE TWIN IS STILL OPEN:** `coot::is_mmcif_filename()` uses the same
...
**Exhibited by:** the phenix.refine mmCIF pair (660 and 659).
**Rule:** `transfer_links_to_mmdb` ignores `con.type` and `fill_links()` pushes every mmdb
LINK into the refinement unfiltered. An H-bond sits at ~2.9 A; a covalent link restraint
pulls toward ~1.4 A. Adjustment (2) filters `Hydrog` before `copy_to_mmdb`.
...
**Exhibited by:** the phenix.refine mmCIF pair -- **and `1AON.cif`
(x15), found 2026-08-12 when the harness was first run against the full corpus.**
...
**Exhibited by:** the phenix.refine pair, `.pdb` / `.cif` (5 chloride ions, written `Cl`).
...
does not**.
**Exhibited by:** the phenix.refine mmCIF pair (35 `HELX_P` rows, 31 sheet ranges,
...
**The residue that looked deliberate and was not:** three of those 35 helices used to be
...
phenix pair now agrees with its PDB sibling on every HELIX and SHEET record. A count that
differs is still printed rather than silently absorbed, but the count no longer differs.
**Assertion:** CHECKED, by `--header-check`: the phenix pair is the case it exists for, and
...
|---|---|---|---|
| 5E1N | MSE 1, MPD 2, CA 4, HOH 9 | 1, 2, 3, 4 BAD | 1, 2, 4, 9 OK |
| 3NYD | 3NY 3, ACT 4, SO4 7, HOH 11 | 2, 3, 4, 5 BAD | 3, 4, 7, 11 OK |
| 3K0N | HOH 2 | 2 OK by luck | 2 OK |
...
**Exhibited by:** the phenix.refine mmCIF pair.
...
whole file. On the phenix pair that is **132 matches as author ids vs 45 as label ids**, and it is
...
**Exhibited by:** converting one of the phenix pair (660 `hydrog`) to PDB yields **0** LINK
records.
...
writes five** (the phenix pair: `2.38654 0.98676 1.51165 ...`).
**Exhibited by:** the phenix pair, and only them -- hence ~7000 of 8740 atoms differing in every
...
result by 8*pi^2, so any small-molecule CIF stating no displacement parameter had the **hardcoded
...
**Corpus:** every `*_hierarchy.cif`, `3K0N`, `5E1N`, `6DMH` and both of the phenix pair have
...
and 15 out, on both one of the phenix pair and `1AON.cif`. The plan's stated reason for
...
**And then it was found AGAIN, in the half the assertion could not see (2026-08-20).**
`GetResolution()` only knows **REMARK 2**, which is a wwPDB *deposition* record -- and
**phenix.refine output has none**. The phenix pair's `.pdb` states its resolution only inside
...
missed every phenix one: the converted mmCIF still had no resolution anywhere. Seen in
...
phenix files; with it restored, neither. Note it does not move chain A's `9 / 5` -- both files
...
**Read side** (`gemmi-mmdb-diff`, no arguments), `0.2.0.0-u51`:
```
summary: 7 identical, 22 with differences, 12 failed to load
```
The seven identical are `1AON.pdb`, `2GEW.pdb`, `3K0N.pdb`, `5E1N.pdb`, `compact/1BNA.cif`,
`compact/1BNA.pdb` and `compact/3ZD5.pdb`.

**The twelve `failed to load` are all path A (raw mmdb) and all expected.** They are not one
thing, and a changed count should be read against the right group:
- **seven structure-factor files** (`*-sf.cif` for 2GEW, 3K0N, 6CH7, 9PMT, 1BNA, 3NIR, 3ZD5) --
  **both** paths refuse them for having zero atoms, which is correct;
- **three chemical-component files** (`small_molecules/ADP.cif`, `AR6.cif`, `CSD.cif`) -- raw
  mmdb cannot read a `chem_comp` file at all (A5c);
- **one small-molecule CIF** (`small_molecules/4517425.cif`) -- likewise unreadable by mmdb;
- **`1AON.cif`** -- mmdb's `ReadCoorFile` fails on it outright.

**Was `6 identical, 17 with differences, 8 failed to load` at `u63`.** The move is the corpus
REORGANISATION, not a code change: the corpus gained three subfolders (`compact/`,
`small_molecules/`, `heterogeneity/`), a set of private fixtures and two local comparison
artifacts were removed, and the harness learned to RECURSE -- without which the eleven files in
those subfolders were invisible to all four gates, silently.
Earlier still it was `3 identical, 16 with differences, 4 failed to load` at `u34`.
**Before reading a changed baseline as a regression, check whether the corpus changed shape** --
list the corpus directory -- because it has done so several times.
...
**Write side** (`gemmi-mmdb-diff --write-check`), `0.2.0.0-u51`:
```
write-side summary: 17 clean, 0 lossy, 0 write-failed, 8 read-declined, 16 skipped (not mmCIF)
```
Was `14 clean, 0 lossy, 0 write-failed, 3 read-declined, 14 skipped` at `u63`; the move is the
corpus reorganisation described above. `skipped` counts files that are not mmCIF at all plus the
chemical-component files, which `is_coordinate_mmcif()` skips -- see A5c. `read-declined` counts
the structure-factor files, which have no atoms.
**`0 lossy` and `0 write-failed` have still never moved.** They are the numbers that matter.

**Round trip** (`gemmi-mmdb-diff --round-trip`), `0.2.0.0-u51`:
```
  A  PDB->mmCIF->mmCIF : 11 model preserved, 5 differ
  B  mmCIF round trip  : 17 stable, 0 NOT stable   <-- the gate
  C  mmCIF->PDB->PDB   : 17 model preserved, 0 differ
```
**B and C both IMPROVED at the reorganisation** -- B was `14 stable` and C was `12 preserved,
2 differ` at `u63`. The new corpus entries cleared two previously-differing cases rather than
merely adding coverage, and the two that used to differ in chain C were private fixtures now
removed from the corpus (C2 and C6).
Chain B is the gate and has never had a `NOT stable`. Chain A's five differences are `2RSF.pdb`
(A3), the chemical-component files (a cell is present in A and not in B), and the small-molecule
entries, whose first hop is a cross-format synthesis -- so differing is correct there.
**Re-measured at `u80` with the resolution and the ADPs compared, and it held** -- but only
after a fix: with the resolution first compared, chain A read `0 model preserved, 14 differ`,
because **E9** had ten PDB inputs silently losing it on conversion. The counts above are the
numbers after that was fixed, not the numbers the stricter comparison first produced.
...
**Header synthesis** (`gemmi-mmdb-diff --header-check`), `0.2.0.0-u51` -- new 2026-08-20:
```
header-check summary: 13 pair(s), 11 agree, 2 differ, 12 mmCIF with no PDB sibling
```
Was `13 agree, 0 differ, 4 mmCIF with no PDB sibling` at `u80`. **The two `differ` are ACCEPTED
DEVIATIONS, not outstanding faults** -- recorded immediately below. The twelve without a sibling
are the structure-factor, small-molecule and chemical-component entries.

**`11 agree` is the number to watch**, and it is worth knowing where it started: `2 agree,
11 differ`, and all eleven were B15-B18, every one a real defect in the synthesis.

**ACCEPTED DEVIATION 1 -- `1BNA`** (`AUTHOR` 1 -> 2, `COMPND` 4 -> 3, `SOURCE` 2 -> 0). A
synthetic-DNA construct. `SOURCE 2 -> 0` is the substantive one and is correct behaviour: there
is no source organism to name, so the synthesis writes no SOURCE record, while the deposition
carries two lines of synthetic-construct prose. The AUTHOR and COMPND counts differ only by
line wrapping.

**ACCEPTED DEVIATION 2 -- `3ZD5`** (`COMPND` 8 -> 6). Line-wrapping only; the text agrees.

Deliberate, annotated count differences also remain INSIDE an agreeing pair, and do not move
the summary: REVDAT (ours states every revision the mmCIF lists) and JRNL, where the `"; "`
author separator rewraps -- visible on `9PMT`, which still agrees.
**Here too the exit status is a gate**: an unannotated difference means the synthesis and the
deposition disagree, and the deposition is right.
...
## WARNING: The gate's remaining blind spot
